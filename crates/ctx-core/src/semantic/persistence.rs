...
pub(super) fn clean_workspace_cache(config: &SemanticPersistenceConfig) -> Result<(), CtxError> {
...
pub(super) fn read_json<T: for<'de> Deserialize<'de>>(path: &Path) -> Result<T, CtxError> {
...
pub(super) fn write_synced(path: &Path, bytes: &[u8]) -> Result<(), CtxError> {
...
pub(super) fn write_atomic(path: &Path, bytes: &[u8]) -> Result<(), CtxError> {
...
#[cfg(not(windows))]
pub(super) fn rename_replace(from: &Path, to: &Path) -> Result<(), CtxError> {
...
pub(super) fn sync_dir(path: &Path) -> Result<(), CtxError> {
